...
bool TestPatchMatch::run() {
...

  {
    ...
    const ImageDimensions sourceDims{ sourceImage.height(),
                                      sourceImage.width() };
    const QImage targetImage(path2);
    const ImageDimensions targetDims{ targetImage.height(),
                                      targetImage.width() };
    ...
    TEST_ASSERT(ImageIO::readImage<3>(path1, pyramid.levels[0].guide.source,
                                      ImageFormat::RGB, 0));
    TEST_ASSERT(ImageIO::readImage<3>(path1, pyramid.levels[0].style.source,
                                      ImageFormat::RGB, 0));
    TEST_ASSERT(ImageIO::readImage<3>(path2, pyramid.levels[0].guide.target,
                                      ImageFormat::RGB, 0));
    TEST_ASSERT(ImageIO::readImage<3>(path2, pyramid.levels[0].style.target,
                                      ImageFormat::RGB, 0));
    ...
    patchMatcher.randomlyInitializeNNF(pyramid.levels[0].forwardNNF,
                                       pyramid.levels[0]);
    float totalError = 0;
    for (int col = 0; col < pyramid.levels[0].forwardNNF.sourceDimensions.cols;
         col++) {
      for (int row = 0;
           row < pyramid.levels[0].forwardNNF.sourceDimensions.rows; row++) {
        float error = 0;
        ImageCoordinates coords = { row, col };
        errorCalc.calculateError(
    ...
    for (int i = 0; i < 2; i++) {
      patchMatcher.patchMatch(configuration, pyramid.levels[0].forwardNNF,
                              pyramid, 2, 0, false, false);
      float totalError = 0;
      for (int col = 0;
           col < pyramid.levels[0].forwardNNF.sourceDimensions.cols; col++) {
        for (int row = 0;
             row < pyramid.levels[0].forwardNNF.sourceDimensions.rows; row++) {
          float error = 0;
          ImageCoordinates coords = { row, col };
    ...
    ImageIO::writeImage("./Examples/patchMatchTest1Output.png",
                        pyramid.levels[0].style.target, ImageFormat::RGB, 0);
  }

  // test whether patchmatch can copy an image using patches from a similar
  // image
  {
    ...
    patchMatcher.patchMatch(configuration, pyramid.levels[0].forwardNNF,
                            pyramid, 6, 0, false, false);

    NNFApplicatorCPU<float, 3, 3> imageMaker;
    imageMaker.applyNNF(configuration, pyramid.levels[0]);
    ImageIO::writeImage("./Examples/patchMatchTest2Output.png",
                        pyramid.levels[0].style.target, ImageFormat::RGB, 0);
  }

  // test blacklisting
  // the entire brown1 image will be constructed from the bottom half of the
  // image.
  {
    const QString path1("./Examples/brown1.png");
    ...
    for (int col = 0; col < pyramid.levels[0].forwardNNF.sourceDimensions.cols;
         col++) {
      for (int row = 0;
           row < .5 * pyramid.levels[0].forwardNNF.sourceDimensions.rows;
           row++) {
    ...
    patchMatcher.patchMatch(configuration, pyramid.levels[0].forwardNNF,
                            pyramid, 6, 0, false, false, &blacklist);
    ...
    for (int col = 0; col < pyramid.levels[0].forwardNNF.sourceDimensions.cols;
      ...
      for (int row = 0;
           row < pyramid.levels[0].forwardNNF.sourceDimensions.rows; row++) {
        if (pyramid.levels[0]
                .forwardNNF.getMapping(ImageCoordinates{ row, col })
                .row <
            .5 * pyramid.levels[0].forwardNNF.sourceDimensions.rows) {
          invalidMappings++;
        }
      }
    }
    std::cout << "Total invalid mappings (patches on blacklist that were still "
                 "mapped to): " << invalidMappings << std::endl;

    NNFApplicatorCPU<float, 3, 3> imageMaker;
    imageMaker.applyNNF(configuration, pyramid.levels[0]);
    ImageIO::writeImage("./Examples/patchMatchTest3Output.png",
                        pyramid.levels[0].style.target, ImageFormat::RGB, 0);
  }

  std::cout << "PatchMatch tests ran, make sure error values decreased and "
               "check output images" << std::endl;
  return true;
}
