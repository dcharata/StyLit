...
bool ImageIO::getImageDimensions(const QString &path, ImageDimensions &dimensions) {
  QImage qImage(path);
  if (qImage.isNull()) {
    return false;
  }
 ...
 */
int ImageIO::floatToChar(float value) { return std::min(255, int(value * 256)); }
...
int ImageIO::floatsToPixel(float r, float g, float b, float a) {
  return (floatToChar(a) << 24) + (floatToChar(r) << 16) + (floatToChar(g) << 8) + floatToChar(b);
}
