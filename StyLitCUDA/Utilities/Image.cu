...
template <typename T> bool Image<T>::populate(const std::vector<InterfaceImage<T>> &images) {
  ...
  check(cudaMemcpy2D((void *)deviceData, pitch, hostImage, hostPitch, hostPitch, rows,
