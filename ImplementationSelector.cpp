...
bool ImplementationSelector::runWithConfiguration(
  ...
  // Picks the right CPU implementation.
...
bool ImplementationSelector::runCPU(const Configuration &configuration) {
  ...
  if (configuration.numGuideChannels > 20 ||
      configuration.numStyleChannels > 4) {
  ...
  return implementations[configuration.numGuideChannels - 1]
                        [configuration.numStyleChannels - 1]
