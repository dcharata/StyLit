...
#pragma omp parallel for schedule(dynamic)
    for (int row = 0; row < nnfError.nnf.sourceDimensions.rows; row++) {
      for (int col = 0; col < nnfError.nnf.sourceDimensions.cols; col++) {
        ...
        if (blacklist != nullptr &&
            blacklist->getMapping(nnf.getMapping(ImageCoordinates{row, col})) ==
  ...
  void initOmega(const Configuration &configuration, std::vector<float> &omega,
                 ImageDimensions dims, ImageDimensions nnfShape, NNF &nnf,
                 int PATCH_SIZE) {
    ...
    for (int row = 0; row < nnfShape.rows; row++) {
      for (int col = 0; col < nnfShape.cols; col++) {
        ...
        updateOmegaValue(configuration, omega, coords.row, coords.col, dims,
                         PATCH_SIZE, 1);
      }
    }
  }
...
#pragma omp parallel for schedule(dynamic)
   ...
   * propagates information throughout the NNF by having each element of the NNF
   * consider mapping to a patch in the codomain that is close to the element's
   * neighbor's mappings.
   * @param configuration the configuration StyLit is running
  ...
  void propagationStep(
    ...
    ErrorCalculatorCPU<T, numGuideChannels, numStyleChannels> errorCalc =
        ErrorCalculatorCPU<T, numGuideChannels, numStyleChannels>();
    float newPatchError1 =
        -1.0; // we know if a patch was out of bounds if its error remains -1,
    ...
    if (domainNeighbor1.within(
            nnf.sourceDimensions)) { // if the neighbor in the nnf domain
                                     // actually exists
      ...
      // (row, col) domain patch to NOTE: the -offset below is from the ebsynth
      // implementation, and is not part of the original patchmatch algorithm
      // This modification seems to reduce blurriness when we average the
      // contents of the NNF into a final image and makes the total error of the
      ...
      if (newPatch1.within(
              nnf.targetDimensions)) { // if new codomain patch is in the
                                       // codomain dimensions
    ...
    if (domainNeighbor2.within(
            nnf.sourceDimensions)) { // if the neighbor in te nnf domain
                                     // actually exists
      ...
      if (newPatch2.within(
              nnf.targetDimensions)) { // if the new codomain patch is in the
                                       // codomain dimensions
        newPatch2Available =
            (blacklist == nullptr) ||
   ...
   * element of the NNF search for a better mapping by repeatedly randomly
    ...
    for (int i = 0; i < radii.size(); i++) {
      ...
      if (newCodomainPatch.within(
        ...
        if (newCodomainPatchAvailable) { // it is only worth to check whether we
          ...
          if (newOmegaError <
              currentOmegaError) { // update the patch that currentPatch maps to
